...
Invio valori il massimo valore che si pu√≤ inviare √® 2^14

*QUINTO BYTE 5:
Byte di parit√† per trovare l'errore
*/
...
void sendByte(char byte0, char byte1, char valore);      //Funzione per inviare dati in cui vengono aggiunti i bit di parit√†
...
void main(void)
{
    ...
    int time;                     //0 ÔøΩ rosso, 1 ÔøΩ verde, 2 ÔøΩ giallo
    ...
    while (1)
    {
        ...
        if (readGateway.Bit)
        {
            ...
            {
            case 0x08: //Se vi √® il comando 0x08 nel primo byte allora prendo solo un pachetto di dati
                ...
                for (unsigned char i = 0; i < 5; i++) //Resetto i byte che ho ricevuto cos√¨ da non continuare ad inviare
                {
                    dataFromGateway[i] = 0;
                }
                break;
            case 0x0A: //Se vi √® il comando 0x0A nel primo byte allora prendo solo un pachetto di dati
                readGatewayDone.Bit = 1;
                readGateway.Bit = 0;
                
                temp = (char)map((ADC_Read(0) >> 2), 0, 255, -20, 60);     //legge la temperatura e la mappa su quei valori
                umidita = (char)map((ADC_Read(1) >> 2), 0, 255, 0, 100);   //legge l'umidit√† e la mappa su quei valori
                ...
                break;
                //Se non vi √® un comando di invio allora mi aspetto i tempi e quindi 15 byte
            default:
        ...
        if (readGatewayDone.Bit)
        {
            ...
            //se c'ÔøΩ stato un timeout resetta la lettura del timeout
            if (readGatewayDone.Timeout)
            {
                readGatewayDone.Timeout = 0;
            }
            //se il readgatewaydone non ÔøΩ stato richiamato dal timeout inizia la modifica dei dati
            else
            {
        ...
        if (secondPassed.Bit && cycled.Bit)
        {
            ...
            {
                if( (*Semafori[i]).times[0]==0 )                            //se per l'i-esimo semaforo Ë stato impostato un tempo 0 allora non Ë utilizzato e viene saltato
                {
                    i++;
                }
                else
                {
                    ...
                    if ((*Semafori[i]).times[lux_select] - time < 0)  //se il timer ha raggiunto il tempo della luce, quindi il countdown Ë terminato...
                    {
                        lux_select++;                                           //...si incrementa il contatore delle luci...
                        time=1;                                                 //...si resetta il timer  
                        if (lux_select >= 3)                                    //Se il contatore delle luci Ë arrivato a 3, ovvero Ë finito il giallo...
                        {
                            lux_select=0;                                       //...resetta il contatore delle luci, tornando al verde...
                            if(i==n_semafori-1)                                 //...e se il ciclo terminato Ë quello dell'ultimo semaforo tutto l'incrocio ha terminato un ciclo...
                            {
                    ...
                    (*Semafori[i]).lux_select=lux_select;             //aggiorna il valore di lÚux_select nel caso sia cambiato
        ...
        case 0:                //==> desplay delle centinaia, porta RA2
            if (centinaia > 0) //mostra la cifra delle centinaia solo se ÔøΩ consistente (maggiore di 0)
            {
                SetDisplay(1, 0, 0, display[centinaia]); //Scrive su "PORTD" i pin che andranno a 1 per far vedere il numero che √® presente nel array "display[*n]"
            }           
            break;
        case 1:                              //==> desplay delle dedcine, porta RA3
            ...
            {
                SetDisplay(0, 1, 0, display[decine]); //Scrive su "PORTD" i pin che andranno a 1 per far vedere il numero che √® presente nel array "display[*n]"
            }
            break;
        case 2:                                  //==> desplay delle unitÔøΩ, porta RA4
            SetDisplay(0, 0, 1, display[unita]); //Scrive su "PORTD" i pin che andranno a 1 per far vedere il numero che √® presente nel array "display[*n]"
            break;
        }
        disp = (disp + 1) % 3; //disp viene incrementato e ha valori tra 0 e 2

        //reset variabili
        //Se ÔøΩ passato un secondo viene impostata a 1 la variabile "cycled" e il timer viene resettato solo al ciclo successivo, quando il codice entra in questo if.
        ...
        //il timer, verrÔøΩ effettuato un ciclo prima di resettare il timer cosÔøΩ da assicurare che quelle porzioni di codice rilevino secondPassed
        if (secondPassed.Bit && cycled.Bit)
        {
            secondPassed.Bit = 0;
            cycled.Bit = 0;
...
void conteggioVeicoli()
{
    ...
    for(unsigned char i=0; i<4; i++)
    {
        ...
        else if (RoadsSensors[i]) //al rilascio controlla e incrementa il mezzo che √® passato
        {
