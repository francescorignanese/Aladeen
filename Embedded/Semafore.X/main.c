...
void main(void)
{
    ...
    while (1)
    {
        ...
        if (secondPassed.Bit && cycled.Bit)
        {
            ...
            do                                                              //Per ogni semaforo calcoler� il countdown per le luci in base alla luce
            {
                ...
                if ((*Semafori[id_semaforo]).times[lux_select] - time < 0)  //se il timer ha raggiunto il tempo della luce, quindi il countdown � terminato...
                {
                    ...
                    if (lux_select >= 3)                                    //Se il contatore delle luci � arrivato a 3, ovvero � finito il giallo...
                    {
                        ...
                        if(id_semaforo==0)                                  //...e se il ciclo terminato � quello del primo semaforo tutto l'incrocio ha terminato un ciclo...
                        {
                ...
                (*Semafori[id_semaforo]).lux_select=lux_select;             //aggiorna il valore di l�ux_select nel caso sia cambiato
